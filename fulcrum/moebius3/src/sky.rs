...
//!
//! Two buffers and one triangle. The first is the frame: the eye, the palette and the handful of
//! numbers the desert is drawn from, as a uniform. The second is the sky, rebuilt every frame:
//! the circles the clouds are made of and the groups over them, in the order they are drawn.
//!
//! The sky goes down as a read-only storage buffer rather than a uniform, which the first version
//! used. A crest here is a union of two to six circles instead of one, so a crowded frame at the
//! widest setting carries several times the circles, and a uniform binding is held to 64 KB. A
//! storage binding is not, and nothing else about the two differs from this shader's point of
//! view: it walks the arrays and never writes to them.
//!
...
//! under the window and under the headless device the `moebius3_still` example builds.

use crate::cloud::{MAX_DISCS, MAX_GROUPS, Sky, Style};
...
/// How wide every line that is not a cloud is, in pixels: the sky's bands, the desert's, the
/// horizon, the rock and the ring around the sun.
///
/// The clouds have their own, on a key, because that is the line the picture is about. The rest
/// of the drawing is the paper it stands on and is left where it was, so that turning the cloud
/// line up thickens the clouds rather than the whole panel.
pub const PICTURE_INK: f32 = 1.8;
...
/// How far a surface has to be turned towards the ground before it is crossed with a second set
/// of hatch lines, as the downward part of its normal.
///
/// The second set is the difference between shade and shadow. A face turned away from the sun is
/// unlit and gets one set of lines; a face turned *down* cannot be lit by anything at all, since
/// there is nothing under the horizon to light it, and a cumulus keeps its darkest value along
/// the undersides of its billows and on the flat base most of all. That is the value this piece
/// was missing when its shading was one set of lines down one side of a silhouette.
///
/// Well over halfway, so an underside has to be a real underside rather than a flank. Set lower
/// and most of every billow's shaded half comes out crossed, since the shaded half of a ball is
/// also leaning downwards, and a drawing where the darkest value is everywhere has no darkest
/// value.
pub const SHADE_UNDER: f32 = 0.62;

/// The angle the hatch lines are drawn at, measured from the line to the sun, in radians.
///
/// A hand hatches at a slant and keeps the same slant across a drawing, so this is one number for
/// the whole sky rather than something worked out per cloud. Off the vertical by enough to read
/// as pen work and off the horizontal by enough not to read as the desert's bands.
pub const SHADE_ANGLE: f32 = 1.05;
...
pub struct Uniforms {
    ...
    /// sun stands, how wide the line around a cloud is in pixels, and how many arcs an element
    /// was built from. The shader reads the first three; the last is there for the readout and
    /// for the test that watches this layout.
    pub counts: [f32; 4],
    /// How far apart the hatch lines run as a fraction of an element's radius, how far a surface
    /// has to be turned towards the ground before it is crossed with a second set, the angle they
    /// are drawn at, and nought for a sky with no shading in it at all.
    pub shade: [f32; 4],
    /// xyz: the direction to the sand under the horse's hooves. w: how tall he is, in radians.
    pub rider: [f32; 4],
    /// Where he is in his stride, how wide the frame is in bearing so the shader can draw the
    /// copy of him that is arriving beside the one that is leaving, and two spare.
    pub gait: [f32; 4],
...
impl Slab {
    /// One, on the heap, since it is ninety kilobytes and has no business on a stack.
...
/// the settings and the window. The binary and the headless example both call it, so a frame on
/// screen and a frame in a PNG cannot drift apart.
///
...
pub fn compose(
    ...
    // Off the same clock as the sky, and worked out here rather than kept anywhere, so a still
    // of any moment has him where he was at that moment. He is folded into the frame, so where
    // the head is pointed and how wide the window is come into it as well as the clock.
    let rider = Rider::at(weather.clock, weather.yaw, game::frame_span(aspect));
    ...
    Uniforms {
        ...
        pen: [PICTURE_INK, SKY_BANDS, GROUND_BANDS, MESA_RISE],
        world: [
            game::PLANET_RADIUS,
        ...
        shade: [
            ...
            SHADE_UNDER,
            SHADE_ANGLE,
            if style.shade { 1.0 } else { 0.0 },
        ],
        rider: [rider.dir[0], rider.dir[1], rider.dir[2], rider.size],
        gait: [rider.gait, rider.span, 0.0, 0.0],
...
impl Renderer {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        let shader = device.create_shader_module(wgpu::include_wgsl!("moebius3.wgsl"));

        let layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("moebius3 layout"),
            entries: &[
                ...
                // The sky, and only the fragment stage reads it. Read-only storage, so the
                // number of circles a frame can carry is not held to a uniform's 64 KB.
                wgpu::BindGroupLayoutEntry {
                    binding: 1,
