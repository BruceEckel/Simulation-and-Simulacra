...
//!
//! ```sh
//! cargo run -p moebius --release --example moebius_still -- still.png 1920 1200 0 400
//! ```
//!
//! The arguments are the file, the size in pixels, which palette, and how far into the weather
//! to look, in seconds. The last one is the whole state of the sky: every circle in the drawing
//! is a function of it, so any moment can be rendered without running up to it.
//!
...
use moebius::sky::{OUTPUT_FORMAT, Renderer, Slab, compose};
...
fn main() {
    ...
    let uniforms = compose(&weather, &sky, &LOOKS[palette], (width, height), &mut slab);
    println!(
