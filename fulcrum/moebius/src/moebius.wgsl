...
struct Uniforms {
    ...
    world: vec4<f32>,
    // how many groups of circles there are, and nothing else yet.
    counts: vec4<f32>,
    ...
    sky_3: vec4<f32>,
    sky_4: vec4<f32>,
    // The four of the desert, underfoot first.
    sand_0: vec4<f32>,
    sand_1: vec4<f32>,
    sand_2: vec4<f32>,
    sun_colour: vec4<f32>,
...
struct SkyBuffer {
    ...
    disc: array<vec4<f32>, 1536>,
}

@group(0) @binding(0) var<uniform> u: Uniforms;
@group(0) @binding(1) var<uniform> s: SkyBuffer;
...
//
// Sixteen blocks around the compass, flat on top and straight down the sides, with the tallest
// of them winning wherever two overlap. Moebius draws his deserts that way: the ground is a
// ruled line with a few slabs standing on it, and every curve in the panel belongs to the sky.
...
// The sand: four flat bands running away to the horizon with a line along each boundary, swung
...
// colour with a bundle of lines along the top; spread by angle, it is four bands with room to be
