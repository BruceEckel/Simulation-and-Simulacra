...
use moebius::cloud::{BANDS, MAX_DISCS, MAX_GROUPS, SMALLEST, Sky, length};
use moebius::game::Weather;
use moebius::look::LOOKS;
use moebius::sky::{Slab, compose, cull};
...
fn the_circles_are_unit_directions() {
    for clock in moments() {
        for disc in &Sky::at(clock).discs {
...
fn a_moment_is_a_moment() {
    ...
    for clock in moments() {
        let first = Sky::at(clock);
        let second = Sky::at(clock);
...
fn the_weather_is_weather() {
    ...
    let early = Sky::at(100.0);
    let late = Sky::at(400.0);
...
fn a_cloud_is_never_swapped_while_it_is_showing() {
    ...
    let mut previous = weight(&Sky::at(0.0));
...
fn a_whole_compass_of_weather_fits_down_the_bus() {
    ...
    for clock in moments() {
        ...
        for turn in 0..24 {
            ...
            let uniforms = compose(&weather, &sky, &LOOKS[0], (2560, 1440), &mut slab);
