...
///
/// Larger than the first version's, because a crest here is a union rather than a circle and the
/// number of circles in one is a setting. `tests/clouds.rs` measures a full compass at
/// [`ARCS_MAX`] against this.
pub const MAX_DISCS: usize = 4608;

/// Fewest circles a crest is built from. Two, because one is a bubble.
pub const ARCS_MIN: u32 = 2;
/// Most.
pub const ARCS_MAX: u32 = 6;
...
/// The two settings that change what is built and what is drawn, rather than what the weather is
/// doing.
///
/// The line weight never reaches this module: a line is drawn along the edge of a shape and the
/// shape does not care how wide it is. It is carried here so that everything a hand on the keys
/// can change is in one place, and `sky.rs` takes the width off it when it fills the buffer.
