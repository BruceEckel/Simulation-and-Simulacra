...
//! under the window and under the headless device the `moebius2_still` example builds.
...
impl Renderer {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        let shader = device.create_shader_module(wgpu::include_wgsl!("moebius2.wgsl"));

        let layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("moebius2 layout"),
            entries: &[
