...
//! The weather is four numbers advanced on the fixed tick, and the clock among them is a float
//! being integrated: the moment that integration picks up the frame's wall-clock delta instead
//! of the fixed one, the sky stops being reproducible and the replay is worth nothing. This
//! catches that, and folds the sky the clock stands for into the digest as well, so a change to
//! how a moment becomes a set of circles shows up here too.
//!
//! The settings are folded in at their defaults rather than scripted. They are read on the frame
//! and not on the tick, because they are a matter of taste rather than of weather, so they are
//! not part of what a replay has to reproduce; what is checked here is that the sky a given
//! setting stands for is the same sky twice.

use fulcrum::prelude::*;
use moebius2::cloud::{Sky, Style};
...
/// Fold the whole of the weather, and the sky it stands for, into one number.
fn digest(app: &mut Fulcrum) -> u64 {
    ...
    // And so is the sky, which is the clock and the settings and nothing else.
    let sky = Sky::at(weather.clock, Style::default());
    eat(sky.groups.len() as u32);
