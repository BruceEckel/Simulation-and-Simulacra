...
//! look in seconds, which way to face in degrees, how many arcs an element is built from, and how
//! wide the line around a cloud is in pixels. The clock is the whole state of the sky: every
//! circle in the drawing is a function of it, so any moment can be rendered without running up to
//! it, and the last two are the settings the keys move while the window is open.
//!
...
use moebius2::game::Weather;
use moebius2::look::LOOKS;
use moebius2::sky::{OUTPUT_FORMAT, Renderer, Slab, compose};
...
fn main() {
    ...
    let style = Style {
        arcs: number(args.next(), Style::default().arcs),
        cloud_ink: decimal(args.next(), Style::default().cloud_ink),
    }
    .clamped();
    ...
    println!(
        ...
        LOOKS[palette].name, style.arcs, style.cloud_ink
    );
}
