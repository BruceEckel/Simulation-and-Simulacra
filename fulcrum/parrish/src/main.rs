//! Parrish, windowed: the keys, the readout and the frame the pass draws into.
//!
...
//!
//! The window opens borderless over the whole display and the painting is computed at **one
...
/// Paint the sky: work out this frame's uniforms and submit the pass.
fn draw(
...
fn readout(
    ...
    let title = format!("PARRISH  {}", look.name);
    ...
    text.value = format!(
        "{title}{pace:>width$}\n\
         glazed once per pixel: {}x{} = {pixels}   {:.2} coats at the zenith   {:.1} ms\n\
         up/down pace   left/right look   Space hold   P palette   H hide   F11 window",
        window.width, window.height, look.sky_depth, screen.pace,
    );
}
