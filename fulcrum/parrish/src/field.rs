...
//! - **It carries its own mip chain.** A cloud plane seen at a grazing angle near the horizon is
//!   minified enormously, and a sheet without mips turns that into a field of sparkling
//!   confetti. wgpu will not build the chain, so it is box-filtered here, wrapping at the edges
//!   like everything else.
//! - **The two cloud channels hold their high octaves harder than usual.** Where the flat-colour
//!   piece next door wants smooth lobes it can lay a line around, this one wants the cauliflower:
//!   a Parrish cumulus is a big simple mass with small crisp knuckles all over its lit side, and
//!   those knuckles are the high octaves of the field surviving instead of being averaged away.
...
const CELLS: [u32; 4] = [3, 5, 2, 13];

/// How many octaves each channel stacks.
const OCTAVES: [u32; 4] = [4, 4, 2, 3];

/// How much of an octave's amplitude the next one keeps.
///
/// A half is the usual answer and it gives a soft, rolling field. The cloud channels hold more
/// than that, which puts real relief at the small scales and is the difference between a bank of
/// fog and a cumulus with knuckles on it. The weather map keeps less: it decides which districts
/// are cloudy and wants no detail whatsoever.
const GAIN: [f32; 4] = [0.46, 0.46, 0.40, 0.50];
...
/// - `a` is fine detail: the roughness on a cloud's edge, the ripple on the water, and the
///   profile of the rock standing on the horizon
pub fn sheet(seed: u32) -> Sheet {
    ...
    for (index, texel) in field.iter_mut().enumerate() {
        ...
        for channel in 0..4 {
            texel[channel] = fbm(
                ...
                OCTAVES[channel],
                GAIN[channel],
                // Wrapping on both halves: the multiply overflows a `u32` from the third channel
                // on, which a release build folds away silently and a debug build panics on.
                // Every arithmetic step that feeds a hash has to say out loud that it means to
                // wrap.
    ...
    // coverage dial with half its travel.
...
/// Gradient noise stacked over `octaves`, starting at `cells` cells to an edge and doubling, each
/// octave keeping `gain` of the last one's amplitude.
pub fn fbm(at: [f32; 2], cells: u32, octaves: u32, gain: f32, seed: u32) -> f32 {
    let mut total = 0.0;
...
/// Gradient noise on a lattice of `cells` cells to an edge, in `-1..=1`, wrapping at the edges of
/// the unit square.
pub fn gradient_noise(at: [f32; 2], cells: u32, seed: u32) -> f32 {
    ...
    // grid of creases once the field is lit.
...
/// A hash of two lattice coordinates and a seed. Written out rather than borrowed from a hasher,
/// because the sky has to come out the same on every machine and in every release.
