...
//!
//! ```sh
//! cargo run -p parrish --release --example parrish_still -- still.png 1920 1200 0 0
//! ```
//!
...
//! question worth asking about a painting is what it looks like, and the answer is a picture.
...
use parrish::field::sheet;
use parrish::game::{WIND, Weather};
use parrish::look::LOOKS;
use parrish::sky::{OUTPUT_FORMAT, Renderer, compose};
...
fn save(
    ...
    // Copies out of a texture want their rows aligned, so the buffer is padded and the padding is
    // dropped again on the way into the PNG.
    let stride = (width * 4).div_ceil(256) * 256;
    let buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("parrish readback"),
